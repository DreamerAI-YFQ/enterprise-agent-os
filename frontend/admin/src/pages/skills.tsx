...
export default function SkillsPage() {
  ...
  const query = useQuery({
    queryKey: ["admin", "skills", scopeFilter],
    queryFn: async () => {
      const params =
        scopeFilter !== "all" ? { params: { query: { scope: scopeFilter } } } : {};
      const { data, error } = await apiClient.GET("/admin/skills", params);
  ...
  const publishMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.POST("/admin/skills/{skill_id}/publish", {
  ...
  const deprecateMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.POST("/admin/skills/{skill_id}/deprecate", {
        params: { path: { skill_id: id } },
        body: { reason: deprecateReason },
      });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "skills"] });
    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-border-subtle px-8 py-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Skill 管理</h1>
            <p className="mt-1 text-sm text-secondary">
              管理所有技能的全生命周期（个人 / 部门 / 公司）
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 pb-8">
        {query.isLoading ? (
          ...
          <EmptyState
            icon={Sparkles}
            title="暂无技能"
            description={search || scopeFilter !== "all" ? "未找到匹配的技能" : "创建一个技能，让 Agent 学会执行特定任务"}
            action={
                    ...
                    <div className="mt-3 rounded-md bg-subtle/50 p-3">
                      ...
                      <div className="mt-2 flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          type="button"
                          variant="danger"
                          onClick={() => deprecateMutation.mutate(skill.id)}
...
function CreateSkillModal({
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      await apiClient.POST("/admin/skills", {
        body: {
          name,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-lg bg-elevated p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-foreground">创建技能</h2>
          <button
            type="button"
            onClick={onClose}
...
function SkillDetailModal({
  ...
  const [departmentId, setDepartmentId] = useState(skill.owner_id ?? "");
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      await apiClient.PUT("/admin/skills/{skill_id}", {
        ...
        body: {
          ...
          instructions,
          scope,
          owner_id: scope === "department" ? departmentId : scope === "personal" ? (skill.owner_id ?? null) : null,
        },
      });
    },
    onSuccess: () => {
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setDepartmentId(skill.owner_id ?? "");
                }}
              >
                取消
              </Button>
