...
def _full_ctx(**overrides) -> ToolExecutionContext:
    defaults = dict(
        tenant_id=TENANT,
        user_id=USER,
        agent_id=AGENT,
...
def _read_ctx(**overrides) -> ToolExecutionContext:
...
class TestToolExecutionContext:
    def test_read_path_passes_with_tenant_only(self):
    ...
    def test_write_path_fails_without_user(self):
        ctx = _full_ctx(user_id=None)  # type: ignore[arg-type]
        with pytest.raises(ValueError, match="user_id"):
            ctx.fail_closed(is_write=True)

    def test_write_path_fails_without_agent(self):
        ctx = _full_ctx(agent_id=None)  # type: ignore[arg-type]
        with pytest.raises(ValueError, match="agent_id"):
            ctx.fail_closed(is_write=True)

    def test_write_path_fails_without_session(self):
        ctx = _full_ctx(session_id=None)  # type: ignore[arg-type]
        with pytest.raises(ValueError, match="session_id"):
            ctx.fail_closed(is_write=True)

    def test_write_path_fails_without_trace(self):
        ctx = _full_ctx(trace_id=None)
        with pytest.raises(ValueError, match="trace_id"):
            ctx.fail_closed(is_write=True)

    def test_write_path_passes_with_full_context(self):
        ctx = _full_ctx()
        ctx.fail_closed(is_write=True)  # should not raise

    def test_department_scope_requires_departments(self):
        ctx = _full_ctx(agent_scope="department", department_ids=[])
        with pytest.raises(ValueError, match="department_ids"):
            ctx.fail_closed(is_write=True)

    def test_department_scope_passes_with_departments(self):
...
class TestContextAdapters:
    def test_to_tenant_context(self):
    ...
    def test_to_guard_context(self):
...
class TestToolInvocation:
    def _invocation(self, **overrides) -> ToolInvocation:
        defaults = dict(
    ...
    def test_intent_digest_stable_across_dict_order(self):
        """Dict key order changes must not change digest."""
        inv1 = self._invocation(canonical_arguments={
            "customer_code": "CUS-001",
            "product_sku": "PRD-002",
            "quantity": 400,
        })
        inv2 = self._invocation(canonical_arguments={
            "quantity": 400,
            "product_sku": "PRD-002",
            "customer_code": "CUS-001",
        })
    ...
    def test_intent_digest_changes_on_value_change(self):
...
class TestIdempotencyKey:
    def test_same_context_same_intent_same_key(self):
        ctx = _full_ctx()
        inv = ToolInvocation(
            tool_name="erp_create_sales_order",
            resource="erp.orders",
