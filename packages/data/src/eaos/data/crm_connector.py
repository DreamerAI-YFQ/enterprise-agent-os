"""CrmConnector — read/write connector for mock CRM tables (crm schema).
...
_CRM_ALLOWED_COLUMNS: dict[str, set[str]] = {
    "leads": {"lead_name", "source", "status", "converted_to_opportunity_id"},
    "opportunities": {
        "opp_name",
        "customer_id",
        "amount",
...
class CrmConnector:
    """DataConnector for mock CRM tables in the ``crm`` schema."""

    SCHEMA = "crm"
    _ALLOWED_RESOURCES: set[str] = {"leads", "opportunities", "activities"}
    _ALLOWED_COLUMNS = _CRM_ALLOWED_COLUMNS
    ...
    async def write(
        ...
        # 2. before snapshot for update/delete
